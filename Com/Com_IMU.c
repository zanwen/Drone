...
void GetAngle(const MPU6050Data *pMpu, FlightAngle *pAngE, float dt)
{
    volatile struct V
    {
        float x;
    ...
    // 加速度归一化
    NormQuat = Q_rsqrt(squa(mpu6050.accelX) + squa(mpu6050.accelY) + squa(mpu6050.accelZ));

    Acc.x = pMpu->accelX * NormQuat;
    Acc.y = pMpu->accelY * NormQuat;
    Acc.z = pMpu->accelZ * NormQuat;
    // 向量差乘得出的值
...

    {
        ...
        *(float *)pAngE = atan2f(2 * NumQ.q1 * NumQ.q2 + 2 * NumQ.q0 * NumQ.q3, 1 - 2 * NumQ.q2 * NumQ.q2 - 2 * NumQ.q3 * NumQ.q3) * RtA; // yaw
        ...
        NormAccz = pMpu->accelX * vecxZ + pMpu->accelY * vecyZ + pMpu->accelZ * veczZ; /*Z轴垂直方向上的加速度，此值涵盖了倾斜时在Z轴角速度的向量和，不是单纯重力感应得出的值*/
    }
}
