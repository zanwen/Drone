...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_3); // RIGHT-1

    // LOG_DEBUG("before start adc with dma")
    // HAL_ADC_Start_DMA(&hadc1, (uint32_t *)&batteryAdcValue, 1);
    // LOG_DEBUG("after start adc with dma")
